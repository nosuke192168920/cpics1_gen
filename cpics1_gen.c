...
#define B21_CFG_LEN 18
#define CONFIG_LEN (B21_CFG_LEN + 3)

#define TITLE_LEN 256

#define HEX_INTERVAL 2

#define TEMPLATE_EXPECT 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, \
        0x11, 0x33, 0x55, 0x77, 0x99, 0xBB, 0xDD, 0xFF, 0x21, 0x43, 0x65, \
        0x87, 0xA9

#define TEMPLATE_START 0x1d6
...
read_config(char *buf, unsigned char *config, char *title)
{
    ...
    // config file format:
    // ( *(VAL) *,){B21_CFG_LEN-1,CONFIG_LEN-1} *(VAL) *# *(.*)$
...
#define OUT_FILE "cpics1prg.hex"
#define OUT_FILE2 "cpics1prgx.hex"

void
version()
{
    printf("CPicS1 program generator (ver %s)\n", VERSION);
...
usage(char **argv)
{
    ...
    fprintf(stderr, "  config:   B21 configuration file (default: %s)\n", CONFIG_FILE);
    ...
    fprintf(stderr, "  -b         enable batch mode\n");
...
main(int argc, char **argv)
{
    ...
    printf("Name: %s\n", title);
    printf("B21 config: ");
    dump(config, B21_CFG_LEN);
    printf("Delay: %d msec\n", config[B21_CFG_LEN] * 10);
    printf("Blink: %s\n", config[B21_CFG_LEN + 1] ? "Yes" : "No");
    printf("Slow:  %s\n", config[B21_CFG_LEN + 2] ? "Yes" : "No");
    ...
    // search template config location in template hex file
